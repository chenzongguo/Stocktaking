...
package com.thl.stocktaking.utils.camera;
...
public final class CameraManager {
	 ...
	 */
	public synchronized void openDriver(SurfaceHolder holder, int CAMERA_FACING)
			throws IOException {
		Camera theCamera = camera;
		if (theCamera == null) {
			// 获取手机背面的摄像头
			theCamera = OpenCameraInterface.open(CAMERA_FACING);
			if (theCamera == null) {
	...
	public synchronized Rect getFramingRect() {
		if (framingRect == null) {
			...
			int width = findDesiredDimensionInRange(screenResolution.x,
					MIN_FRAME_WIDTH, MAX_FRAME_WIDTH);// 绘制的正方形的宽度，注意横竖屏，一般以屏幕的宽（短的）
			// 将扫描框设置成一个正方形
			int height = width;
...
//	}
	...
	public void touchFocus(View v, MotionEvent event){
		CameraHelper.touchFocus(camera, event, v, null);
	}
}
